...
namespace EasyProblems
{
     ...
     */
        public static void CalculateAmount()
        {
            ...
            var valuesArray = inputParams.Split(' ');
            
            var withdrawalAmount = int.Parse(valuesArray[0]);
            var balanceAmount = float.Parse(valuesArray[1]);

            if (withdrawalAmount >= balanceAmount - 0.4)
            {
                //Insufficient fund
                Console.WriteLine($"{balanceAmount:0.00}");
            }
            ...
            {
                Console.WriteLine($"{balanceAmount - withdrawalAmount - 0.5:0.00}");
            }
        }
    }
}
